...
采用三应用微服务架构：**Connector**（游戏 WebSocket 长连接）、**Core**（业务逻辑 + Web API）、**Web**（前端面板），通过 TCP Socket 通信，Connector 独立运行以保证连接不随 Core 重启而断开。
...
- **Connector** 独立进程维持游戏 WebSocket 长连接
- **Core** 通过 TCP Socket 与 Connector 通信，重启 Core 不断开游戏连接
- Connector 自动重连，无需手动干预
...
# 3. 先启动 Connector（游戏连接进程），再启动 Core（业务 + Web）
pnpm dev:connector   # 终端 1
pnpm dev:core        # 终端 2
...
> **注意**：Connector 和 Core 需要分别在两个终端中运行。Connector 维持游戏 WebSocket 长连接，Core 重启不会断开已建立的游戏连接。
...
| 命令                   | 说明                                         |
| ---------------------- | -------------------------------------------- |
| `pnpm build`           | 构建所有应用（web + core + connector）        |
| `pnpm dev`             | 同时启动所有开发服务                          |
| `pnpm dev:core`        | 仅启动 Core（业务逻辑 + Web API）             |
| `pnpm dev:web`         | 仅启动 Web 前端开发服务器                     |
| `pnpm dev:connector`   | 仅启动 Connector（游戏 WebSocket 连接进程）   |
| `pnpm start`           | 生产模式启动 Core                             |
| `pnpm start:connector` | 生产模式启动 Connector                        |
...
- **Connector**：独立 NestJS 进程，维持各账号的游戏 WebSocket 长连接，通过 Protobuf 编解码游戏协议
...
qq-farm-bot-nest/
├── apps/
│   ├── connector/         # 游戏连接进程（NestJS + WebSocket + Protobuf）
│   │   └── src/
│   │       ├── connector/ # TCP Server、连接管理、Proto 加载
│   │       ├── common/    # 常量、工具、调度器
