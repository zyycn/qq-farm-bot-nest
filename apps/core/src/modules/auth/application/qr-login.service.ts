...
import { CookieUtils, HashUtils } from './qr-login.utils'
...
export class QRLoginService {
  ...
  async queryMiniProgramStatus(loginCode: string) {
    ...
    return { status: 'Error' as const, msg: `状态码: ${code}` }
  }
  ...
  async getMiniProgramAuthCode(ticket: string, appid = '1112386029') {
    ...
    if (!code || typeof code === 'number' || RE_NUMERIC_CODE.test(String(code))) {
      this.logger.warn(`ide/login 返回异常: 状态码=${code}, 响应数据=${JSON.stringify(response.data)}`)
      throw new Error(`获取农场登录码失败（状态码=${code}）`)
    }
    return code
  }
}
