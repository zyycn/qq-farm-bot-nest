...
export class AccountTopicsService {
  ...
  async handleSubscribe(
    ...
    if (!client.rooms.has(WsPushService.ROOM_ALL))
      client.join(WsPushService.ROOM_ALL)

    if (resolved) {
      for (const event of eventsSet) {
        if (!this.broadcastOnlyEvents.has(event))
          client.join(WsPushService.roomForEvent(resolved, event))
      }
    }
  ...
  handleUnsubscribe(client: SocketWithMeta, data: Record<string, unknown>): null {
    ...
    for (const event of eventsList) {
      if (accountId)
        client.leave(WsPushService.roomForEvent(accountId, event))
    }
