...
export class WarehouseWorker {
  ...
  async useItem(itemId: number, count = 1, landIds: number[] = []): Promise<any> {
    const body = Buffer.from(this.t.UseRequest.encode(this.t.UseRequest.create({
      item_id: toLong(itemId), count: toLong(count),
    ...
    try {
      ...
      if (!msg.includes('code=1000020') && !msg.includes('请求参数错误')) throw e
  ...
  private isFertilizerRelatedItemId(itemId: number): boolean {
    if (itemId <= 0 || itemId === 1011 || itemId === 1012) return false
    if (FERTILIZER_RELATED_IDS.has(itemId)) return true
  ...
  private getFertilizerItemTypeAndHours(itemId: number) {
    if (NORMAL_FERTILIZER_ITEM_HOURS.has(itemId)) return { type: 'normal' as const, perItemHours: NORMAL_FERTILIZER_ITEM_HOURS.get(itemId)! }
    if (ORGANIC_FERTILIZER_ITEM_HOURS.has(itemId)) return { type: 'organic' as const, perItemHours: ORGANIC_FERTILIZER_ITEM_HOURS.get(itemId)! }
    const info = this.gameConfig.getItemById(itemId) || {} as any
    const interactionType = String(info.interaction_type || '').toLowerCase()
    if (interactionType === 'fertilizer') return { type: 'normal' as const, perItemHours: 1 }
    if (interactionType === 'fertilizerpro') return { type: 'organic' as const, perItemHours: 1 }
  ...
  async autoOpenFertilizerGiftPacks(): Promise<number> {
    try {
      ...
      for (const it of bagItems) {
        const id = toNum(it?.id); const count = Math.max(0, toNum(it?.count))
        if (id > 0 && count > 0 && this.isFertilizerRelatedItemId(id)) merged.set(id, (merged.get(id) || 0) + count)
      }
      ...
      for (const it of bagItems) {
        const id = toNum(it?.id); const count = Math.max(0, toNum(it?.count))
        if (id === NORMAL_CONTAINER_ID) containerHours.normal = count / 3600
        if (id === ORGANIC_CONTAINER_ID) containerHours.organic = count / 3600
      }
      ...
      for (const [itemId, rawCount] of merged) {
        ...
        if (type === 'normal' || type === 'organic') {
          ...
          if (currentHours >= FERTILIZER_CONTAINER_LIMIT_HOURS) continue
          if (perItemHours > 0) {
            ...
            try { await this.batchUseItems([{ itemId, count: useCount }]); opened += useCount; details.push(`${this.gameConfig.getItemName(itemId)}x${useCount}`); if (type === 'normal') containerHours.normal += useCount * perItemHours; else containerHours.organic += useCount * perItemHours }
            catch { continue }
          }
        }
        await sleep(100)
      ...
      const msg = String(e?.message || '')
      if (msg.includes('code=1003002') || msg.includes('化肥容器已')) return 0
  ...
  private getGoldFromItems(items: any[]): number {
    for (const item of (items || [])) { const id = toNum(item.id); if ((id === 1 || id === 1001) && toNum(item.count) > 0) return toNum(item.count) }
    return 0
  }

  async sellAllFruits() {
    if (!this.store.isAutomationOn('sell', this.accountId)) return
    try {
      const bagReply = await this.getBag()
      ...
      for (const item of items) {
        const id = toNum(item.id); const count = toNum(item.count)
      ...
      for (let i = 0; i < toSell.length; i += SELL_BATCH_SIZE) {
        ...
        try {
          ...
          if (gain > 0) serverGoldTotal += gain
        }
        catch {
          for (const it of batch) {
            try { const r = await this.sellItems([it]); const g = Math.max(0, this.getGoldFromItems(r?.get_items || [])); if (g > 0) serverGoldTotal += g } catch {}
          }
        }
      ...
      if (totalGoldEarned > 0) this.client.emit('sell', totalGoldEarned)
    }
  ...
  async getBagDetail() {
    ...
    for (const it of rawItems) {
      const id = toNum(it.id); const count = toNum(it.count)
      if (id <= 0 || count <= 0) continue
      const info = this.gameConfig.getItemById(id)
      let name = info?.name || ''; let category = 'item'
      if (id === 1 || id === 1001) { name = '金币'; category = 'gold' }
      else if (id === 1101) { name = '经验'; category = 'exp' }
      else if (this.gameConfig.getPlantByFruitId(id)) { name = name || `${this.gameConfig.getFruitName(id)}果实`; category = 'fruit' }
      else if (this.gameConfig.getPlantBySeedId(id)) { const p = this.gameConfig.getPlantBySeedId(id); name = name || `${p?.name || '未知'}种子`; category = 'seed' }
      if (!name) name = `物品${id}`
      const interactionType = info?.interaction_type ? String(info.interaction_type) : ''

      if (!merged.has(id)) {
        merged.set(id, {
          id, count: 0, uid: 0, name, image: this.gameConfig.getItemImageById(id), category,
          itemType: info ? (Number(info.type) || 0) : 0, price: info ? (Number(info.price) || 0) : 0,
          level: info ? (Number(info.level) || 0) : 0, interactionType, hoursText: '',
        })
      }
  ...
  async getCurrentTotalsFromBag() {
    const bagReply = await this.getBag()
    const items = this.getBagItems(bagReply)
    let gold: number | null = null; let exp: number | null = null
    for (const item of items) {
      const id = toNum(item.id); const count = toNum(item.count)
      if (id === 1 || id === 1001) gold = count
      if (id === 1101) exp = count
    }
    return { gold, exp }
  }
