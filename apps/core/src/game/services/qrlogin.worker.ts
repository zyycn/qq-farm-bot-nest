...
const QR_PRESETS: Record<string, QRPreset> = {
  vip: {
    name: 'QQ会员 (VIP)', description: 'QQ会员官网',
    aid: '8000201', daid: '18',
  ...
  qzone: {
    name: 'QQ空间 (QZone)', description: 'QQ空间网页版',
...
export class QRLoginService {
  ...
  async requestQRCode(presetKey = 'vip') {
    ...
    const params = new URLSearchParams({
      appid: config.aid, e: '2', l: 'M', s: '3', d: '72', v: '4',
      t: String(Math.random()), daid: config.daid, u1: config.redirectUri,
    })
  ...
  async checkQRStatus(qrsig: string, presetKey = 'vip') {
    ...
    const params = new URLSearchParams({
      ptqrtoken: String(ptqrtoken), from_ui: '1', aid: config.aid, daid: config.daid,
      action: `0-0-${Date.now()}`, pt_uistyle: '40', js_ver: '21020514', js_type: '1',
  ...
  async queryMiniProgramStatus(loginCode: string) {
    ...
    if (response.status !== 200) return { status: 'Error' as const }
    const { code, data } = response.data
    if (+code === 0) {
      if (+data.ok !== 1) return { status: 'Wait' as const }
      return { status: 'OK' as const, ticket: data.ticket, uin: data.uin, nickname: data.nick || '' }
    }
    if (+code === -10003) return { status: 'Used' as const }
