...
export class StatsTracker {
  ...
  operations = {
    harvest: 0, water: 0, weed: 0, bug: 0, fertilize: 0, plant: 0,
    steal: 0, helpWater: 0, helpWeed: 0, helpBug: 0,
    taskClaim: 0, sell: 0, upgrade: 0, levelUp: 0,
  }
  ...
  updateStats(currentGold: number, currentExp: number) {
    if (this.lastState.gold === -1) this.lastState.gold = currentGold
    if (this.lastState.exp === -1) this.lastState.exp = currentExp
