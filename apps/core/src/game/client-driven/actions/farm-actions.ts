...
import type { AnalyticsWorker } from '../../services/analytics.worker'
import type { StatsTracker } from '../../services/stats.worker'
import type { OwnedLandStatus } from '../helpers/land.helpers'
import { Logger } from '@nestjs/common'
import { getLandTypeByLevel, PlantPhase } from '../../constants'
import { sleep, toNum } from '../../utils'
...
export class FarmActions {
  ...
  async fertilize(landIds: number[], fertilizerId = NORMAL_FERTILIZER_ID): Promise<number> {
    const ids = landIds.map(toNum).filter(id => Number.isFinite(id) && id > 0)
    if (!ids.length)
      return 0
